<?php /* Script name: downloadFiles
 * Description: Downloads all the files with a .txt
  * * extension in a directory via FTP. */
include("ftpstuff.inc");
$dir_name = "data/";
$connect = ftp_connect($servername) or die(" Can't connect to FTP server");
$login_result = ftp_login($connect, $userID, $passwd) or die(" Can't log in");
...
foreach($filesArr as $value) {
    ...
    if(!file_exists($value))
    {
        ...
        echo "File $ value already exists!\ n";
    }
}
ftp_close($connect);
?>
